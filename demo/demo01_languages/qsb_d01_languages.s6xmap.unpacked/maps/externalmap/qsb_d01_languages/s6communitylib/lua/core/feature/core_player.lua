...
function Lib.Core.Player:SetPlayerName(_PlayerID, _Name)
    -- If sting key is passed
    if type(_Name) == "string" and _Name:find("^[A-Za-z0-9_]+/[A-Za-z0-9_]+$") then
        if not IsLocalScript() then
            ExecuteLocal([[Lib.Core.Player:SetPlayerName(%d, "%s")]], _PlayerID, _Name);
        else
            local Name = GetStringText(_Name);
            CONST_PLAYER_NAMES[_PlayerID] = Name;
            ExecuteGlobal([[CONST_PLAYER_NAMES[%d] = "%s"]], _PlayerID, Name);
            GUI_MissionStatistic.PlayerNames[_PlayerID] = Name;
        end
    -- If normal name passed
    else
...
function Lib.Core.Player:GetTerritoryName(_TerritoryID)
    ...
    TerritoryName = GetStringText(StringTable .. "/Territory_" .. TerritoryName);
